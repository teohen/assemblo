...
class Parser {
  ...
  parseArgument(arg, lineNum) {
    ...
    if (arg[0] === ".") {
      return new argument_default(tokens_default.ARG_TYPES.LBL, arg, arg);
    }
    if (/^-?[0-9]\d*$/.test(arg)) {
...
class Evaluator {
  ...
  logger;
  labels;
  constructor(inQ, outQ, registers, memory, operations, logger, labels) {
  ...
  jmpFn(args, ln, cond) {
    const arg1 = args[0];
    const arg2 = args[1];
    argument_default.validateType(arg1, [tokens_default.ARG_TYPES.LBL], ln);
    argument_default.validateType(arg2, [
      tokens_default.ARG_TYPES.REG,
      tokens_default.ARG_TYPES.NUM
    ], ln);
    ...
    if (cond(val)) {
      ...
      if (!lbl) {
        throw new Error(`AT LINE: ${ln}. CAN'T JUMP TO INVALID LABEL: ${arg1.intern}`);
      }
      return lbl;
    }
    return ln;
  ...
  lblFn(args, ln) {
    const arg1 = args[0];
    argument_default.validateType(arg1, [tokens_default.ARG_TYPES.LBL], ln);
    const newLbl = { name: arg1.literal, lineNumber: ln };
...
class Program {
  ...
  constructor() {
    ...
    this.evaluator = new evaluator_default([], [], new Map, new Map, [], [], new Map);
  ...
  reset(inQ) {
    ...
    this.parser = new parser_default;
    this.evaluator = new evaluator_default(this.inQ, this.outQ, this.registers, this.memory, this.parser.operations, this.logger, this.labels);
  }
...
var savedCode = localStorage.getItem("code") || `START
  ...code hsere
END`;
editor.setValue(savedCode);
var codemirror_default = editor;
...
function renderCodeInfo(p, inputStack) {
  if (codeInfo)
    codeInfo.children[0].innerHTML = p.line || "_";
  if (codeInfo)
    codeInfo.children[1].innerHTML = p.status || "_";
  if (codeInfo)
    codeInfo.children[2].innerHTML = "0";
  if (codeInfo)
    codeInfo.children[3].innerHTML = inputStack ? "[" + inputStack + "]" : "[]";
  if (codeInfo)
    codeInfo.children[4].innerHTML = p.outQ ? "[" + p.outQ + "]" : "[]";
}
function renderRegistersMemoryInfo(p) {
  if (memoryTable)
    memoryTable.children[0].innerHTML = p.memory.get("MX0") !== undefined ? p.memory.get("MX0") : "_";
  if (memoryTable)
    memoryTable.children[1].innerHTML = p.memory.get("MX1") !== undefined ? p.memory.get("MX1") : "_";
  if (memoryTable)
    memoryTable.children[2].innerHTML = p.memory.get("MX2") !== undefined ? p.memory.get("MX2") : "_";
  if (registersTable)
    registersTable.children[0].innerHTML = p.registers.get("R0X") !== undefined ? p.registers.get("R0X") : "_";
  if (registersTable)
    registersTable.children[1].innerHTML = p.registers.get("R1X") !== undefined ? p.registers.get("R1X") : "_";
  if (registersTable)
    registersTable.children[2].innerHTML = p.registers.get("R2X") !== undefined ? p.registers.get("R2X") : "_";
}
