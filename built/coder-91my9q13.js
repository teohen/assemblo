...
class Argument {
  ...
  static validateType(arg, expTypeList, ln) {
    ...
    if (!valid) {
      throw new Error(`AT LINE: ${ln}. INVALID ARGUMENT TYPE: ${arg.type}, EXPECTED: ${expTypeList}`);
    }
    return valid;
  }
...
class Parser {
  ...
  parseArgument(arg, lineNum) {
    ...
    if (/^-?[0-9]\d*$/.test(arg)) {
      return new argument_default(tokens_default.ARG_TYPES.NUM, parseInt(arg), parseInt(arg));
    }
    throw new Error(`AT LINE: ${lineNum}. UNKNOWN ARGUMENT: ${arg}`);
  }
  parseLine(line, num) {
    const op = new operation_default(num, undefined, [], undefined);
    const ONE_ARGS = ["PRT"];
    ...
    if (this.procedures.includes(tokens_default.FUNCTIONS[fPart])) {
      op.type = tokens_default.FUNCTION_TYPES.PROC;
    } else {
      op.type = tokens_default.FUNCTION_TYPES.FLOW;
    }
    op.funcName = tokens_default.FUNCTIONS[fPart];
...
class Evaluator {
  ...
  popFn(args, ln) {
    ...
    if (!argument_default.validateValue(arg2, tokens_default.LISTS.INPUT)) {
  ...
  pushFn(args, ln) {
    ...
    if (!argument_default.validateValue(arg1, tokens_default.LISTS.OUTPUT)) {
  ...
  jmpNegFn(args, ln) {
    ...
    if (arg1.literal > this.operations.length || arg1.literal < 0) {
      throw new Error(`AT LINE: ${ln}. CAN'T JUMP TO INVALID LINE: ${arg1.literal}`);
    }
...
var myModal = new bootstrap.Modal(testModal);
...
var runDelay2 = document.getElementById("runDelay").firstElementChild;
...
submitBtn2.addEventListener("click", () => {
  ...
  p.run(code, () => {
    ...
    ui_default.showModal(p, p.test(expected));
    ui_default.updateUI(p, inputStack);
  }, 1);
});
var autoSaveTimeout;
codemirror_default.on("change", function(instance, changeObj) {
  if (autoSaveTimeout)
